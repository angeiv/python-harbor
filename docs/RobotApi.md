...
# **refresh_sec**
> RobotSec refresh_sec(body, robot_id, x_request_id=x_request_id)
...
# create an instance of the API class
api_instance = harbor.RobotApi(harbor.ApiClient(configuration))
body = harbor.RobotSec() # RobotSec | The JSON object of a robot account.
robot_id = 56 # int | Robot ID
x_request_id = 'x_request_id_example' # str | An unique ID for the request (optional)

try:
    # Refresh the robot secret
    api_response = api_instance.refresh_sec(body, robot_id, x_request_id=x_request_id)
    pprint(api_response)
except ApiException as e:
